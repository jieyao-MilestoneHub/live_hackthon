...
export const MOCK_HIGHLIGHTS: Highlight[] = [
  {
    ...
    selected: true,
    locked: false,
    // Rich explainability fields (why this moment) — normally filled by the
    // chat-analysis backend; mocked so the "為何入選" panel demos offline.
    signal: 'fusion',
    status: 'included',
...
const PROGRESS_ANALYSIS: { at: number; step: string; message: string }[] = [
  { at: 0, step: 'UPLOAD_RECEIVED', message: '已接收影片，準備進場分析。' },
  { at: 400, step: 'VALIDATING', message: '正在驗證來源影片編碼與時間基準。' },
  { at: 1000, step: 'TRANSCRIBING', message: '正把直播音訊轉為逐字稿並分離說話者。' },
