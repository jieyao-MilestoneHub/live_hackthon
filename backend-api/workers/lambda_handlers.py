...
def starter(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
    ...
    for record in event.get("Records", []):
        ...
        # AND is explicitly analysis_source=transcribe. In the full-auto chat
        # S3-drop flow the project may not exist yet when source.mp4 lands (chat.csv
        # auto-creates it), and it must never be a chat project — either way the
        # Transcribe run would race/clobber the chat pipeline (and fails outright on
        # >2 GB videos). The video-only flow creates the project (analysis_source=
        # "transcribe", the default) via the API BEFORE the upload completes, so it
        # still proceeds here.
        project = get_repository().get_project(project_id)
